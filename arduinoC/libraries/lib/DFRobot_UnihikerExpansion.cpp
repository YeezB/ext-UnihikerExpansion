...
float DFRobot_UnihikerExpansion::getDHTValue_tem(eIONumber_t number)
{
    sDhtData_t dhtData = getDHTValue(number);
    return dhtData.temperature;
}

float DFRobot_UnihikerExpansion::getDHTValue_hum(eIONumber_t number)
{
    sDhtData_t dhtData = getDHTValue(number);
...
float DFRobot_UnihikerExpansion::getvalue(eIONumber_t number, eIOType_t mode)
{
    if(mode == eADC){
       return getADCValue(number);
    }else if(mode == eDHT11){
        return getDHTValue_tem(number);
    }else if(mode == eDHT11_hum){
        return getDHTValue_hum(number);
    }else if(mode == eDHT22){
        return getDHTValue_tem(number);
    }else if(mode == eDHT22_hum){
        return getDHTValue_hum(number);
    }else if(mode == eDS18B20){
        return get18b20Value(number);
    }else if(mode == eReadGpio){
        return getGpioState(number);
    }
  
}


void DFRobot_UnihikerExpansion::motorRun(eMotorNumber_t number, uint8_t speed, uint8_t dir)
{
  if (number == eMotor1) {
    setMotorDuty(eMotor1_A, dir == 0 ? speed : 0);
    setMotorDuty(eMotor1_B, dir == 1 ? speed : 0);
  } else if (number == eMotor2) {
    setMotorDuty(eMotor2_A, dir == 0 ? speed : 0);
    setMotorDuty(eMotor2_B, dir == 1 ? speed : 0);
  } else if (number == eMotor3) {
    setMotorDuty(eMotor3_A, dir == 0 ? speed : 0);
    setMotorDuty(eMotor3_B, dir == 1 ? speed : 0);
  } else if (number == eMotor4) {
    setMotorDuty(eMotor4_A, dir == 0 ? speed : 0);
    setMotorDuty(eMotor4_B, dir == 1 ? speed : 0);
  } else if (number == eMotorall) {
    for (int i = eMotor1; i <= eMotor4; i++) {
      setMotorDuty(eMotor1_A, 0);
      setMotorDuty(eMotor1_B, 0);
      setMotorDuty(eMotor2_A, 0);
      setMotorDuty(eMotor2_B, 0);
      setMotorDuty(eMotor3_A, 0);
      setMotorDuty(eMotor3_B, 0);
      setMotorDuty(eMotor4_A, 0);
      setMotorDuty(eMotor4_B, 0);
    }
  } else {
    for (int i = eMotor1; i <= eMotor4; i++) {
      setMotorDuty(eMotor1_A, 0);
