...
public:
  ...
  float getDHTValue_tem(eIONumber_t number);
  float getDHTValue_hum(eIONumber_t number);
  /**
   * @fn: get18b20Value
 ...
 /**
  ...
  void motorRun(eMotorNumber_t number, uint8_t speed, uint8_t dir);
  /**
   * @fn: getSr04Distance
  ...
  float getvalue(eIONumber_t number, eIOType_t mode);
private:
